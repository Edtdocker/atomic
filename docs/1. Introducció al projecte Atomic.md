...
Permet mantenir la ultima versió del sistema operatiu disponible per a rollback, per tant, si hi ha algun problema, es pot accedir a la versió anterior presionant la tecla SHIFT durant l'arrencada del sistema, sel·leccionant la opció alternativa en el menú d'arrencada.	
També és posible fer un rollback desde la versió actual mitjançant `#atomic rollback`. la orde atomic és un alias de rpm-ostree.	
